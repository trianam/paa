...
\begin{frame}
  \frametitle{Solution}
  \begin{itemize}
  \item A \alert{quadratic} B-Spline (grade 2, order 3) is contained
    inside the union of \alert{convex hulls} composed of consecutive
    \alert{triples} of vertexes of control polygon.\pause
  \end{itemize}
  \begin{center}
    \begin{tikzpicture}
      \coordinate (a) at (0,0);
  ...
  \pause
  \begin{block}{Idea}
    If we use a \alert{quadratic} B-Spline to smooth the
    path, we can be sure that it don't intersect obstacles if we keep
    free from obstacles all the triangles with vertexes tree
    consecutive points.
  \end{block}
\end{frame}
